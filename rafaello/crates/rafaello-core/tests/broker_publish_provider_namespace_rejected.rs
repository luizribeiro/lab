//! Plugin publishing on `provider.*` is rejected as
//! `PublishOnReservedNamespace` (scope §B3 step 3, c10; pi-1 §10).
//! m2 staged: even provider-bound plugins are rejected because m2's
//! spawn refuses to register provider plugins per SP4 #7.
