//! c24 — V3 lock-side publish ACL: `frontend.*` rejected.
...
#[test]
fn lock_publish_on_frontend_namespace_is_rejected() {
    let a = canonical("github.com/acme:alpha@1.0.0");
    let plug = entry_with_publishes(&["alpha"], false, None, &["frontend.tui.user_message"]);
