//! Scope §TR3 step 1 / c12 — with neither a value-match hit nor a
//! referenced-union hit, the canonical taint reduces to the
//! provider-identity entry only.
...
#[tokio::test]
async fn reemit_tool_request_no_matches_keeps_provider_only_taint() {
    ...
    assert_eq!(
        taint, &expected,
        "no matches → provider-identity only canonical taint: {taint:?}",
    );
