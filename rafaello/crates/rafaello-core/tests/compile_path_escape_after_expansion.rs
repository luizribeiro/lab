//! c31 — §C3 negative: `${project}/../../etc` expands to a path
//! that escapes the substituted project root. The lexical
//! containment check fires before the filesystem walk →
...
#[test]
fn parent_traversal_after_expansion_rejected() {
    ...
    bundles.insert(
        ...
        GrantBundle {
            filesystem: Some(GrantFilesystem {
                read_dirs: vec!["${project}/../../etc".to_owned()],
    ...
    assert!(
        matches!(err, CompileError::PathEscape),
