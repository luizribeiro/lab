//! Scope §A10 / c12 — when a cited `in_reply_to[*]` has fallen out of
//! the `ReferencedTaintIndex.by_result_id` cache (TTL elapsed), the
//! lookup returns `None` and the referenced-union arm contributes
//! empty. Fail-open per §A10: the canonical taint reduces to
//! `provider-identity ∪ value_match`, no error.
...
async fn reemit_tool_request_referenced_result_expired_from_cache_fails_open() {
    ...
    // Advance past TTL so the recorded result-entry is dropped on next lookup.
    ...
    assert_eq!(
        taint, &expected,
        "expired referenced entry must fail-open empty — provider-only canonical taint",
    );
