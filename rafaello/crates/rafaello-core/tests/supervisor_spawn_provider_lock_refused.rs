//! c16 SP4 Phase A step 7 — a `PluginAcl` with
//! `provider_id.is_some()` (i.e. `bindings.provider = true`)
//! is refused with `InvalidPlan { reason: ProviderNotInM2 }`.
...
#[tokio::test]
async fn spawn_with_provider_lock_returns_provider_not_in_m2() {
    ...
    match err {
        ...
        other => panic!("expected ProviderNotInM2, got {other:?}"),
    }
