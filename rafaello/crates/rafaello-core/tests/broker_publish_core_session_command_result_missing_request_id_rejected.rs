...
//! / §SL0 suffix-list extension for slash-command results.
...
#[test]
fn publish_core_command_result_missing_request_id_rejected() {
    let acl = BrokerAcl {
    ...
    match err {
        BrokerError::MissingRequestId { topic, .. } => {
            assert_eq!(topic, "core.session.command_result");
        }
