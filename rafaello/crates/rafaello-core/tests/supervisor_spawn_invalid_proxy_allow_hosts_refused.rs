//! c16 SP4 Phase A step 5 — a malformed `allow_hosts` entry
//! under `NetworkPlan::Proxy` surfaces as
//! `InvalidPlan { reason: NetworkAllowHostsInvalid }`. (pi-1 B4.)
...
#[tokio::test]
async fn spawn_with_invalid_proxy_allow_hosts_returns_invalid_plan() {
    ...
    let plan = CompiledPlugin {
        ...
        network: NetworkPlan::Proxy {
            allow_hosts: vec!["not a valid host pattern!".to_string()],
        },
        env: EnvPlan::default(),
    ...
    match err {
        SpawnError::InvalidPlan {
            canonical: c,
            reason: InvalidPlanReason::NetworkAllowHostsInvalid { source: _ },
        } => {
            assert_eq!(c, canonical);
        }
        other => panic!("expected NetworkAllowHostsInvalid, got {other:?}"),
    }
