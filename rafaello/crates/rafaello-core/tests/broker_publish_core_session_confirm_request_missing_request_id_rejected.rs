//! `publish_core_with_taint("core.session.confirm_request", _,
//! request_id=None, …)` is rejected with `MissingRequestId` — scope §CT2
//! suffix-list extension applies to the confirmation request topic.
...
#[test]
fn publish_core_confirm_request_missing_request_id_rejected() {
    let acl = BrokerAcl {
    ...
    match err {
        BrokerError::MissingRequestId { topic, .. } => {
            assert_eq!(topic, "core.session.confirm_request");
        }
