//! c26 — `bindings.renderer_kinds = ["text"]` (built-in name)
//! rejected by V3's M7 mirror.
...
#[test]
fn builtin_renderer_kind_in_lock_is_rejected() {
    let a = canonical("github.com/acme:alpha@1.0.0");
    let mut e = entry(&[], false, None);
    e.bindings.renderer_kinds = vec!["text".to_string()];
