//! `plugin.<own>.rpc_reply` publishes must carry `in_reply_to` — a
//! missing field is rejected as `InvalidInReplyTo { reason: Missing }`
//! (scope §B6, c11). Mirrors the `tool_result` arity check.
