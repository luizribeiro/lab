//! `publish_core_with_taint("core.session.command_result", _,
//! request_id=Some, in_reply_to=None, …)` is rejected as
//! `InvalidInReplyTo { Missing }` — scope §CT3 / §SL0 mandatory
//! `in_reply_to` extension for slash-command results.
...
#[test]
fn publish_core_command_result_missing_in_reply_to_rejected() {
    let acl = BrokerAcl {
