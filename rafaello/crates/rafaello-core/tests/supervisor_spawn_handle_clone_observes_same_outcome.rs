//! c21 SP4 Phase B step 20 — `SpawnHandle` is `Clone` and every
//! clone observes the same `Arc<ReaperOutcome>` once the reaper
//! task publishes terminal status (pi-1 B2 / pi-5 §1).
...
async fn spawn_handle_clones_observe_same_outcome() {
    ...
    let outcome_a = tokio::time::timeout(Duration::from_secs(5), handle.wait())
        .await
        .expect("wait a timed out");
    let outcome_b = tokio::time::timeout(Duration::from_secs(5), twin.wait())
        .await
        .expect("wait b timed out");
    assert!(
        Arc::ptr_eq(&outcome_a, &outcome_b),
        "clones must observe the same Arc<ReaperOutcome>"
    );
}
