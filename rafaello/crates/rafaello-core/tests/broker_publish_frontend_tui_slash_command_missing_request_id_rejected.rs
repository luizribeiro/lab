//! Frontend publishing `frontend.tui.slash_command` with
//! `request_id: None` is rejected as `MissingRequestId` — scope §SL0
//! suffix-list extension. Slash-command is a root event so only
//! `request_id` is mandatory (no `in_reply_to`).
...
#[test]
fn frontend_slash_command_missing_request_id_rejected() {
