//! c21 SP4 Phase B steps 18–20 — first end-to-end spawn through
//! `PluginSupervisor::spawn` of the c20 fixture binary in
//! `respond_peer_call` mode. Verifies the `SpawnHandle` lifecycle:
//! canonical/topic accessors, pre-exit `try_wait`, ready signal,
//! SIGTERM, and a terminal `ReaperOutcome::Exited`.
//!
//! NOT the canonical scope-named headline test (pi-2 B5): the
//! two-fixture publish/observer demo lands in c30.
...
/// Grant the sandbox enough exec/read authority to launch the
/// dynamically-linked fixture binary. Falls back to `/nix/store`
/// (the canonical lab dev environment) plus the binary's parent
/// directory when `LOCKIN_TEST_EXEC_DIRS` is unset.
...
async fn spawn_fixture_respond_peer_call_lifecycle() {
    ...
    assert_eq!(handle.topic_id(), real_topic.as_str());
    assert!(handle.try_wait().is_none(), "child must not have exited");
    ...
    assert!(
        handle.child_pid().is_none(),
        "child_pid must clear after wait observes outcome"
    );
}
