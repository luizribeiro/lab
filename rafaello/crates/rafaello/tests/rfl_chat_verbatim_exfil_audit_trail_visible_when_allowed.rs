//! c24 / scope §EXFIL2 + §A5 — companion to c23's headline integration
//! test. Same fixture + scripted stub response, but the TUI scripted
//! answers are `allow,allow`: the operator allows fetch, then allows
//! the verbatim `send-mail` whose `to`+`body` reproduce the
//! exfil strings. Mailcat receives the call; the persisted entries +
//! audit rows + mailcat log + fetch log are asserted against the live
//! `${PROJECT_ROOT}/.rafaello/state/` and `.rafaello-plugin-data/`
//! trees.
//!
//! ## Deviation (mirrors c23)
//!
//! The c23 stub fixture (`exfil-stub-response.json`) returns a single
//! chat completion carrying *both* tool_calls (`web-fetch` + `send-mail`).
//! `rfl-openai` (m5a) currently publishes both
//! `provider.openai.tool_request` events back-to-back, before either
//! `core.session.tool_result` lands; `core.session.tool_result` does
//! not currently cause the openai adapter to issue a follow-up chat
//! completion. The taint-match map therefore has no fetch
//! tool_result recorded by the time send-mail's args reach the
//! broker, so the §AL1 predicate cannot fire and the
//! `confirm_request_taint_attached` audit row this row's scope text
//! cites as the "regression anchor" is not written end-to-end.
//!
//! Per c24's row text (paths 1/2/3): path 1 (extend rfl-openai to
//! issue a follow-up chat completion after tool_results land) is a
//! non-trivial production change with cross-test ripple risk
//! (existing tests script single completions and rely on no
//! follow-up); path 2 (synthesise the bus events in-tree) skips the
//! `rfl chat` orchestrator and forfeits the end-to-end framing this
//! row's acceptance bullets pin (mailcat log, entries table,
//! through-the-orchestrator audit rows). c24 therefore lands on
//! **path 3 (accept the deviation)**: the end-to-end assertions
//! that *can* land — mailcat captures the verbatim call, both
//! tool_results land `ok: true`, audit shows two `confirm_allowed`
//! rows — do land, and the `confirm_request_taint_attached` row
//! is documented here as the scope.md §EXFIL2 acceptance gap.
//! This deviation is routed to the m5b retrospective §3 / §5.
...
#[test]
#[serial(rfl_chat)]
fn rfl_chat_verbatim_exfil_audit_trail_visible_when_allowed() {
    ...
    assert_eq!(
        ...
        Some(EXFIL_EMAIL),
        "mailcat captured verbatim recipient; line={:?}",
        mailcat_lines[0]
    );
    assert_eq!(
        mailcat_payload["args"]["body"].as_str(),
        Some(EXFIL_URL),
        "mailcat captured verbatim body; line={:?}",
        mailcat_lines[0]
    );
    ...
    // §EXFIL2 acceptance gap (documented in this file's header): the
    // `confirm_request_taint_attached` row is not asserted here. Under
    // the current single-completion stub + m5a `rfl-openai` flow, both
    // tool_calls publish before the fetch tool_result lands, so the
    // §AL1 predicate cannot fire for send-mail's modal. Tracked in
    // the m5b retrospective.
}
