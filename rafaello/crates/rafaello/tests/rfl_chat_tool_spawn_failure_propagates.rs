...
#[test]
fn rfl_chat_tool_spawn_failure_propagates() {
    ...
    assert!(
        ...
        "stderr missing ToolSpawnFailed: {stderr}"
    );
    assert!(
        !stderr.contains("ProviderSpawnFailed"),
        "stderr should not mention ProviderSpawnFailed (provider should succeed): {stderr}"
    );
}
