//! c25 / scope §EXFIL3 + §AL1 + §TR4b — third companion to the c23/c24
//! verbatim-exfil integration tests. Same fixture lock (openai +
//! mailcat + rafaello-fetch) but the scripted stub response makes the
//! model propose a turn-2 `send-mail` with a body the LLM
//! **fabricated** (no substring overlap with the fetch result, no
//! shared scalar) AND no `in_reply_to` chain back to the fetch
//! tool_result. Neither the §A4 value-match arm nor the §TR4b
//! referenced-union arm of canonical-taint enrichment fires for the
//! send-mail confirm_request. With operator answers `allow,allow`,
//! both modals fire, both tool_results land `ok: true`, mailcat
//! captures the fabricated call — and the §AL1 predicate is **false**
//! for both modals: zero `confirm_request_taint_attached` audit rows
//! are written.
//!
//! ## Provider-only-taint framing
//!
//! Under the m5a single-completion `rfl-openai` flow (see c24's
//! deviation header) both `provider.openai.tool_request` events are
//! published before any `core.session.tool_result` lands, so the
//! `in_reply_to` derivation in `rfl-openai` (from
//! `state.observed_tool_results`) yields `[]` for both legs. That
//! lines up with c25's intent: §TR4b's referenced-union arm has
//! nothing to union, and §A4's value-match arm finds no overlap
//! because the fabricated body shares no >=16-byte substring with the
//! fetch content. The canonical taint vector that reaches each
//! confirm_request is therefore the bare provider-identity vector
//! attached by the broker (§7.2.2: `[{source: "provider", detail:
//! "openai"}]`). The §AL1 predicate ("at least one non-provider
//! entry") is false, and the
//! `confirm_request_taint_attached` audit row is not written for
//! either modal — exactly the negative coverage §EXFIL3 / §AL1's
//! "predicate false" branch pins.
...
const FETCH_BODY: &str = "Volcanic activity observed near remote southern islands";
const FABRICATED_TO: &str = "bob@example.org";
const FABRICATED_BODY: &str = "Quarterly earnings report attached for review";

#[test]
#[serial(rfl_chat)]
fn rfl_chat_no_value_match_keeps_provider_only_taint() {
    ...
    assert_no_shared_match_substring(FETCH_BODY, FABRICATED_BODY);
    assert_no_shared_match_substring(FETCH_BODY, FABRICATED_TO);
    ...
    let output = Command::new(workspace_bin("rfl"))
        ...
        .env("RFL_TUI_TEST_CONFIRM_ANSWERS", "allow,allow")
        .env("RFL_TUI_MAX_LIFETIME", "15")
        .env("LITELLM_API_KEY", "sk-test-no-match")
    ...
    assert_eq!(
        ...
        Some(true),
        "send-mail tool_result ok=true on allow; entry={:#?}",
        mail_result
    );
    ...
    let mailcat_log_path = project_root
        ...
        .join("mailcat.log");
    let mailcat_raw = std::fs::read_to_string(&mailcat_log_path).expect("read mailcat.log");
    let mailcat_lines: Vec<&str> = mailcat_raw.lines().filter(|l| !l.is_empty()).collect();
    assert_eq!(
        mailcat_lines.len(),
        1,
        "expected one mailcat.log entry on allow; got {mailcat_lines:#?}"
    );
    let mailcat_payload: Value =
        serde_json::from_str(mailcat_lines[0]).expect("mailcat.log line is JSON");
    assert_eq!(
        mailcat_payload["args"]["to"].as_str(),
        Some(FABRICATED_TO),
        "mailcat captured the fabricated recipient; line={:?}",
        mailcat_lines[0]
    );
    assert_eq!(
        mailcat_payload["args"]["body"].as_str(),
        Some(FABRICATED_BODY),
        "mailcat captured the fabricated body; line={:?}",
        mailcat_lines[0]
    );
    ...
    let confirm_allowed_count = audit_kinds
        .iter()
        .filter(|k| k.as_str() == "confirm_allowed")
        .count();
    assert_eq!(
        confirm_allowed_count, 2,
        "expected two confirm_allowed rows (fetch + mail); got {audit_kinds:?}"
    );
    assert!(
        !audit_kinds.iter().any(|k| k == "confirm_denied"),
        "no confirm_denied row expected on allow,allow; got {audit_kinds:?}"
    );

    // §EXFIL3 / §AL1 "predicate false" headline: the canonical taint
    // for both confirm_request modals is provider-only, so the §AL1
    // predicate ("at least one non-provider entry") does not fire and
    // no `confirm_request_taint_attached` row is written.
    let taint_attached_count = audit_kinds
        .iter()
        .filter(|k| k.as_str() == "confirm_request_taint_attached")
        .count();
    assert_eq!(
        taint_attached_count, 0,
        "expected zero confirm_request_taint_attached rows (provider-only taint); got {audit_kinds:?}"
    );
}

/// Guardrail: the test premise — that the §A4 value-match arm cannot
/// fire for the fabricated send-mail args against the fetch body —
/// only holds while no >=16-byte substring is shared (router default
/// `DEFAULT_TAINT_MATCH_SUBSTRING_MIN_BYTES`). Asserted at test start
/// so a future edit to the constants can't silently invalidate the
/// negative coverage.
fn assert_no_shared_match_substring(a: &str, b: &str) {
    const MIN: usize = 16;
    let bytes_a = a.as_bytes();
    let bytes_b = b.as_bytes();
    if bytes_a.len() < MIN || bytes_b.len() < MIN {
        return;
    }
    for window in bytes_a.windows(MIN) {
        assert!(
            !bytes_b.windows(MIN).any(|w| w == window),
            "FETCH_BODY and fabricated arg share a {MIN}-byte substring; \
             this would allow §A4 value-match to fire and invalidate the test"
        );
    }
}
...
fn stub_response_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests")
        .join("fixtures")
        .join("no-match-stub-response.json")
}
