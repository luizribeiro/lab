...
fn rfl_chat_demo_bar() {
    ...
    let finalized_count = stderr
        .lines()
        .filter(|l| {
            l.starts_with("rfl-tui: bus.event topic=core.session.entry.finalized")
                && l.strip_prefix("rfl-tui: bus.event topic=core.session.entry.finalized")
    ...
    assert_eq!(
        finalized_count, 9,
        "expected 9 entry.finalized bus.event lines; stderr={stderr}"
    );
}
