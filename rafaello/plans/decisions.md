...
| 22 | 2026-05-07 | Bidirectional `PeerHandle` (notify + call, both directions over one fd) is promoted from fittings "Future work" to a v1 requirement. | Bus broker fan-out cannot be implemented from `ServiceContext::notify` alone, and `renderer.render` / helper-spawn / future tool-call modes need core→plugin request/response (pi review 1 finding 1; pi review 2 blocking finding 1). Overview §15.6. Supersedes the round-1 `ServerHandle::notify`-only proposal. | proposed | — |
