...
| Confirmation protocol | Core-mediated three-topic flow (`core.session.confirm_request`, `frontend.<id>.confirm_answer`, `core.session.confirm_reply`) used to obtain explicit user consent for sink calls and other gated actions. |
...
| Taint | Structured `[{source, detail}, …]` provenance attached to bus events on `core.session.tool_result` and `core.session.tool_request`; populated by core, never trusted from plugins. |
