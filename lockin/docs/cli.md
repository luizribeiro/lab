...
## Tracing (`lockin trace`)

`lockin trace` runs a program under your `lockin.toml` policy in real
enforcement and writes every access the sandbox denied to a file.
Where `lockin infer` answers "what does this program touch?",
`lockin trace` answers "what does my policy block?".

```sh
lockin trace [-o OUT] [-c CONFIG] -- PROGRAM ARGS...
```

Flags:

- `-o, --output PATH` — where to write the denial log. Default
  `./lockin-denials.log`. Overwritten per run, no append.
- `-c, --config PATH` — `lockin.toml` to enforce. If omitted,
  resolution mirrors run-mode: `./lockin.toml` if it exists, else a
  deny-all default (which makes for a noisy first trace).
- `--` — separates lockin's flags from the program. Same as run-mode
  and infer.

Output format — header explaining what the file is, one line per
denial, footer summary:
...
Op tokens: `read`, `stat`, `readdir`, `write`, `create`, `delete`,
`exec`. Same vocabulary as `[filesystem]` in `lockin.toml`, so adding
a corresponding rule is a direct translation.

Two-terminal workflow for TUI programs (`vim`, `htop`, anything that
takes the terminal): run `lockin trace` in one terminal, `tail -f
lockin-denials.log` in another. `lockin trace` itself only writes two
status lines to stderr — start ("recording denials to …") and end
("N denial(s) recorded") — so it doesn't fight the child for the tty.

Caveats:

- A single trace only captures paths exercised in that run. Iterate
  with different inputs to broaden coverage.
- `lockin trace` runs the program under your real policy. If the
  policy is too tight, the program may fail mid-run. That is the
  point — the failure surfaces in the log.
- Allow events are not recorded; only denials.
- macOS `mach-lookup`, `sysctl-read`, `file-ioctl`, network operations
  outside the lockin schema are not represented (consistent with
  `lockin infer`).

Backward compatibility:

- Existing run-mode and `lockin infer` invocations are unchanged.
- `trace` is reserved as the first positional token only. If your
  program is literally named `trace`, invoke as `lockin -- trace args`
  or `lockin ./trace args`; both bypass the subcommand dispatch.

Exit codes mirror run-mode: the traced program's exit code on
success, 125 on a lockin internal error.

Concurrency: multiple concurrent `lockin trace` runs do not
cross-contaminate. Each run gets a fresh UUID; the audit channel is
scoped to that UUID.

Platform notes:

- **Linux** uses `syd` with `default/<categories>:deny` plus your
  policy's allow rules; denied accesses arrive on the run's
  dedicated audit fd as JSONL with `act:"deny"`.
- **macOS** uses `sandbox-exec` with `(deny default (with message
  (param "RUN_ID")))` plus your policy's allow rules. The kernel
  Sandbox.kext auto-publishes deny lines to `log stream`; `lockin
  trace` filters that stream by RUN_ID.
...
| `sandbox.allow_interactive_tty` | `bool` | Allow access to the calling process's controlling terminal — `/dev/tty` (the kernel's per-process alias for the controlling terminal) and the inherited stdio tty paths. Does not grant access to other tty devices on the system. Linux: also enables a fixed allowlist of termios/window-size ioctls (TCGETS/TCSETS family, TIOCGWINSZ/TIOCSWINSZ, FIONREAD). |
