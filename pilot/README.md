...
- **First-turn vs resume**: drivers whose CLI uses different flags (gemini, pi) override `resume_command`. `Session` tracks observed `TurnItem::Complete` count; failed turns don't bump the counter, so retry-on-failure transparently re-uses `command()`.
