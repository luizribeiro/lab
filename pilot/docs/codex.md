...
| `sandbox`             | `SandboxMode`           | `SandboxMode::DangerFullAccess` | Maps to `--sandbox` — see [Sandbox modes](#sandbox-modes).                                                |
| `dangerously_bypass_approvals` | `bool`         | `true`                 | Pass `--dangerously-bypass-approvals-and-sandbox`. Disables codex's approval gate so headless turns run unattended. Set to `false` to restore it — see [Approval flow](#approval-flow). |
...
- **Sandbox:** `SandboxMode::DangerFullAccess` — codex can write anywhere by default. Pilot also passes `--dangerously-bypass-approvals-and-sandbox` via `CodexConfig.dangerously_bypass_approvals`. Pilot does not sandbox; see [docs/sandboxing.md](sandboxing.md) for the recommended approach (lockin / capsa).
...
|----------------------------------|-----------------------|----------------------------------------------------------------------------------------------|
| `SandboxMode::DangerFullAccess`  | `danger-full-access`  | No sandbox; codex can read and write anywhere the child process can reach. This is pilot's default. |
| `SandboxMode::WorkspaceWrite`    | `workspace-write`     | Writes inside the workdir are allowed; writes outside the workdir are blocked.               |
| `SandboxMode::ReadOnly`          | `read-only`           | Codex can read the workdir but writes are blocked.                                           |

To restore codex's built-in restriction, configure a more conservative variant explicitly. Out-of-process sandboxing should use a dedicated tool — see [docs/sandboxing.md](sandboxing.md).
...
Codex maintains an approval gate that is independent of the sandbox: even with `WorkspaceWrite`, the CLI can still pause to ask a human to confirm individual tool calls. Pilot is headless, so by default it passes `--dangerously-bypass-approvals-and-sandbox` to skip that gate — `CodexConfig.dangerously_bypass_approvals` defaults to `true`.

Set `dangerously_bypass_approvals: false` on `CodexConfig` to restore codex's approval gate. The flag is now a typed knob; earlier versions of pilot surfaced it only through `TurnOptions::extra_args`, and that workaround is no longer needed. Treat the flag the same way you would treat `BypassPermissions` on Claude: only leave it on in sandboxed or otherwise expendable working directories.
