...
fn stdio_e2e_initialize_list_and_call_follow_strict_jsonrpc_envelopes() {
    ...
    assert_eq!(initialize["result"]["serverInfo"]["name"], "mcpfit-example");
...
fn stdio_e2e_invalid_tool_arguments_return_error_envelope() {
    ...
    assert!(
        ...
        "error message should describe argument problem, got: {:?}",
        response["error"]["message"]
    );
}
...
"#;
    ...
    assert!(
        response["error"]["message"]
            .as_str()
            .is_some_and(|m| m.contains("before initialize")),
        "error message should describe lifecycle violation, got: {:?}",
        response["error"]["message"]
    );
}
