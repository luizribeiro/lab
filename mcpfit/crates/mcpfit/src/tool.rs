...
mod tests {
    ...
    fn description_overrides_previous_value() {
        let tool = Tool::new("add").description("first").description("second");
    ...
    async fn call_deserializes_typed_args() {
        let tool = Tool::new("add")
            .handler(|args: AddArgs, _cx| async move { Ok::<_, McpfitError>(args.a + args.b) });
        let response = tool
    ...
    fn input_for_unit_emits_empty_object_schema() {
        ...
        assert_eq!(tool.input_schema_value(), Some(&json!({"type": "object"})));
    }
