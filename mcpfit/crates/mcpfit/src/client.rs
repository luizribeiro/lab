...
impl ProgressRegistry {
    ...
    pub(crate) fn register(&self, token: ProgressToken, sender: ProgressSender) -> Arc<AtomicU64> {
...
mod tests {
    ...
    async fn initialize_sends_typed_request_and_decodes_result() {
        ...
        let server = tokio::spawn(async move {
            ...
            let params = request
                .params
                .expect("initialize request must carry params");
            assert_eq!(
    ...
    async fn router_records_missed_progress_when_per_call_buffer_is_full() {
        ...
        use super::{PROGRESS_CHANNEL_CAPACITY, ProgressToken};
        use crate::protocol::ProgressNotificationParams;
        ...
        assert!(
            client.progress_registry().missed_counter(&token).is_some(),
    ...
    async fn tool_call_handle_awaits_final_response_on_success() {
        ...
        let pending =
            fittings_client.start_call("tools/call", json!({"name": "ping", "arguments": {}}));
    ...
    async fn tool_call_handle_maps_is_error_to_tool_failed() {
        ...
        let fittings_client = FittingsClient::connect(OneShotConnector::new(client_transport))
            ...
            .expect("fittings client connects");
        let pending =
            fittings_client.start_call("tools/call", json!({"name": "explode", "arguments": {}}));
        let (_tx, rx) = mpsc::channel::<ProgressNotificationParams>(4);
        let missed = Arc::new(AtomicU64::new(0));
        let handle = ToolCallHandle::new(pending, rx, missed, None, None);
    ...
    async fn awaiting_progress_handle_removes_registry_entry() {
        ...
        let progress_token = ProgressToken::from_value(&token).expect("token convertible");
        assert!(
