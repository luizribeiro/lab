...
use fittings::{
    FittingsError, RequestEnvelope, ResponseEnvelope, encode_response_line, to_error_envelope,
};
    ...
    T: fittings::core::transport::Transport + Sync + 'static,
{
    let mut workers = tokio::task::JoinSet::<(
        JsonRpcId,
        std::result::Result<Value, FittingsError>,
        Arc<AtomicBool>,
    )>::new();
...
pub(crate) fn classify_entrypoint(env_fittings: Option<&str>, args: &[String]) -> EntrypointAction {
...
mod tests {
    use super::{
        CancellationConfig, EntrypointAction, MCP_CANCELLATION_METHOD,
        MCP_CANCELLATION_REQUEST_ID_FIELD, Server, classify_entrypoint,
    };
    use crate::tool::{Tool, ToolSpec};
