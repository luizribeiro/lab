...
use crate::context::{Cx, ProgressSink, extract_progress_token};
...
pub trait McpService {
    ...
    async fn call_tool(&self, ctx: ServiceContext, params: ToolsCallParams)
    -> Result<ToolResponse>;
...
impl McpServiceImpl {
    ...
    pub(crate) async fn dispatch(
        ...
        match method {
            "initialize" => {
                ...
                let result = self.initialize(ServiceContext::detached(), p).await?;
                serde_json::to_value(result).map_err(|e| FittingsError::internal(e.to_string()))
            }
            "notifications/initialized" => {
                self.initialized(ServiceContext::detached(), params).await
            }
            "tools/list" => {
                let result = self.list_tools(ServiceContext::detached(), params).await?;
                serde_json::to_value(result).map_err(|e| FittingsError::internal(e.to_string()))
            }
            "tools/call" => {
            ...
            "tools/register" => {
                let p: ToolsRegisterParams = serde_json::from_value(params)
                    .map_err(|e| FittingsError::invalid_params(e.to_string()))?;
                let result = self.register_tool(ServiceContext::detached(), p).await?;
                serde_json::to_value(result).map_err(|e| FittingsError::internal(e.to_string()))
            }
    ...
    async fn call_tool_with_cancellation(
        ...
        match handler {
            Some(handler) => handler(params.arguments, cx)
                .await
...
#[cfg(test)]
mod tests {
    use super::{McpService, McpServiceImpl, Ordering, SessionLifecycle, mcp_service_schema};
    use crate::error::McpfitError;
    use crate::protocol::{InitializeParams, ServerInfo, ToolsCallParams, ToolsRegisterParams};
    use crate::registry::ToolRegistry;
    use crate::response::ToolResponse;
    use crate::tool::Tool;
    use fittings::ServiceContext;
    use fittings::serde_json::{Value, json};
    ...
    async fn dynamic_tools_knob_does_not_expose_register_method() {
        ...
        svc.initialized(ServiceContext::detached(), json!({}))
            .await
            .unwrap();
        let err = svc
            .register_tool(ServiceContext::detached(), register_params("ping"))
            .await
    ...
    async fn initialized_transitions_to_running() {
        ...
        let result = svc
            .initialized(ServiceContext::detached(), json!({}))
            .await
    ...
    async fn list_tools_before_initialize_is_rejected() {
        let svc = service("demo", "0.1.0");
        let err = svc
            .list_tools(ServiceContext::detached(), json!({}))
            .await
    ...
    async fn call_tool_before_initialize_is_rejected() {
        let svc = service("demo", "0.1.0");
        let err = svc
            .call_tool(ServiceContext::detached(), call_params("a"))
            .await
    ...
    async fn call_tool_before_initialized_notification_is_rejected() {
        ...
        assert!(err.to_string().contains("before notifications/initialized"));
        assert_eq!(
            svc.lifecycle_state(),
            SessionLifecycle::AwaitingInitializedNotification
        );
    ...
    async fn call_tool_unknown_tool_returns_method_not_found() {
        ...
        svc.initialized(ServiceContext::detached(), json!({}))
            ...
            .unwrap();
        let err = svc
            .call_tool(ServiceContext::detached(), call_params("missing"))
            .await
    ...
    async fn register_tool_is_rejected_before_initialize() {
        let svc = service("demo", "0.1.0");
        let err = svc
            .register_tool(ServiceContext::detached(), register_params("late"))
            .await
    ...
    async fn register_tool_when_enabled_adds_callable_tool() {
        ...
        let response = svc
            .call_tool(ServiceContext::detached(), call_params("ping"))
            .await
    ...
    async fn register_tool_emits_list_changed_notification_on_success() {
        ...
        svc.initialized(ServiceContext::detached(), json!({}))
            .await
            .unwrap();
        svc.register_tool(ServiceContext::detached(), register_params("ping"))
            .await
