...
//!
//! Thin layer on top of `crate::proc` that knows how to build the two
//! concrete daemon children (`capsa-netd`, `capsa-vmm`), pass their
//! launch specs, and wait for either one to exit. There is no generic
//! trait here on purpose: we have exactly two daemons and the old
//! `DaemonAdapter` abstraction was paying for flexibility we never
//! used.
...
/// Paths netd needs to read at runtime (DNS config, cgroup/cpu info).
/// Carried on the sandbox policy; not part of the launch-spec contract.
...
impl VmConfig {
    /// Start the VM. Spawns netd (if networking is configured) and the
    /// vmm child inside sandboxes, waits for either to exit, and tears
    /// the other one down.
    pub fn start(&self) -> Result<()> {
        self.validate().context("invalid VM configuration")?;

        if self.interfaces.is_empty() {
            return run_vmm_only(self);
        }

        run_with_network(self)
    }
}
...
fn run_with_network(config: &VmConfig) -> Result<()> {
    ...
    // When multi-interface support lands, turn this into a loop over
    // `config.interfaces` that builds `Vec<NetInterfaceSpec>` and
    // `Vec<ResolvedNetworkInterface>` in lockstep.
    debug_assert_eq!(
    ...
    let mut vmm =
        spawn_vmm(&vmm_spec, vec![guest_fd]).context("failed to spawn sandboxed VMM process")?;
...
/// Block until netd sends the one-byte readiness signal. Deadline-based
/// so `EINTR` retries do not extend the total wait beyond `timeout`.
/// Fixes audit finding #2.
...
fn spawn_vmm(spec: &VmmLaunchSpec, guest_fds: Vec<OwnedFd>) -> Result<ChildHandle> {
    ...
    // Inherit each guest fd and record its kernel-assigned number on
    // the final spec. This is the one-stage replacement for the old
    // two-stage "placeholder zeros then adapter overwrites" pattern
    // (audit finding #12).
...
// ── integration tests ────────────────────────────────────────────────
//
// These tests exercise the entire VmConfig::start path through real
// sandboxed (or bypass) spawns against shell-script fake daemons.
// They replace the ~450 lines of generic `DaemonSupervisor` tests
// that lived under the old `daemon/supervisor.rs` module.
...
mod tests {
    ...
    fn make_temp_executable_script(prefix: &str, body: &str) -> PathBuf {
        ...
        // the argument to `--launch-spec-json`. Extract `ready_fd` from
        // that JSON so the body can reference it as `$READY_FD` without
        // hardcoding a kernel-assigned fd number.
        let prelude = r#"
case "$1" in
  --launch-spec-json)
...
"#;
    ...
    fn process_exists(pid: u32) -> bool {
        // SAFETY: `kill(pid, 0)` does not send a signal; it is only
        // used for existence probing.
    ...
    fn no_network_start_reports_vmm_spawn_failure() {
        ...
        // resolve_binary now catches non-executable files up front, so
        // the error points at resolution rather than `Command::spawn`.
        let rendered = format!("{err:#}");
        assert!(
